...
export class CalculationTaskModel {
  static async create(data: Omit<CalculationTask, 'id' | 'createdAt' | 'updatedAt' | 'completedAt'>): Promise<CalculationTask> {
    const result = await pool.query(
      `INSERT INTO calculation_tasks (
  ...
  static async findById(id: string): Promise<CalculationTask | null> {
    const result = await pool.query('SELECT * FROM calculation_tasks WHERE id = $1', [id]);
  ...
  static async findAll(filters?: {
    ...
    limit?: number;
    offset?: number;
  }): Promise<{ tasks: CalculationTask[]; total: number }> {
  ...
  static async update(id: string, data: Partial<CalculationTask>): Promise<CalculationTask | null> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramCount = 0;
    ...
    const result = await pool.query(
      `UPDATE calculation_tasks SET ${fields.join(', ')} WHERE id = $${paramCount} RETURNING *`,
      values
    );
  ...
  private static mapRowToTask(row: any): CalculationTask {
    return {
      id: row.id,
      name: row.name,
